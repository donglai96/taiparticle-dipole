...
if __name__ == '__main__':
    if len(sys.argv) > 1:
        print("The runing mode", sys.argv[1:])
    ...
    wave_name = id + '/wave_info.npy'
    with open(wave_name,'rb') as f:
        Bw_lat= np.load(f)[0,:]
        lats = np.load(f)
         # phi is the phi_particle - phi_wave
    ...
    if len(sys.argv) <= 1:
        fig,axs = plt.subplots(1,4,sharex = 'col',figsize = (8,4))
        print('Making individual particles info!')
        for ip in ips:
        ...
        pitch_angle_equator_final = np.rad2deg(get_equator_pitchangle(pitch_angle_rad[-1,:],rz[-1,:]))
        fig, axs = plt.subplots(1, 2)
        axs[0].hist(pitch_angle_equator_final,bins = 20)
        axs[1].plot(np.rad2deg(lats),Bw_lat)
        plt.savefig(id+'dist.jpg',format = 'jpg')
        plt.show()
        print(lats.shape,Bw_lat.shape)